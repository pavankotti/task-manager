...
import { FolderKanban, LogOut, Bell, PartyPopper, X } from 'lucide-react';
...
export const Layout = ({ children }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      setDismissedBroadcastId(localStorage.getItem('dismissed_broadcast_id'));
    }
  }, []);
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900">
      ...
      {/* SIDEBAR */}
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col justify-between z-10 shrink-0">
        <div>
          ...
          <nav className="p-4 space-y-1">
            ...
            <Link 
              ...
              className={`flex items-center gap-3 px-3 py-2 rounded-md text-sm font-semibold transition-colors ${
                pathname.includes('/projects') || pathname.includes('/board')
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        {/* COMPLETED TASK BROADCAST BANNER */}
        {broadcast && broadcast.assignee && broadcast.id !== dismissedBroadcastId && (
          <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-4 py-2 flex items-center justify-between gap-2 text-sm font-medium shadow-sm shrink-0">
            <div className="flex-1 flex items-center justify-center gap-2">
              <PartyPopper size={16} className="text-yellow-300 animate-bounce" />
              <span>
            ...
            <button 
              ...
              className="text-white hover:text-indigo-200 p-0.5 rounded transition-colors cursor-pointer shrink-0"
