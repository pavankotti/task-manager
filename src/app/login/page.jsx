...
export default function LoginPage() {
  ...
  useEffect(() => {
    // If already logged in, send to projects page
    if (user) {
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow-sm border border-slate-200 sm:rounded-xl sm:px-10">
          <form className="space-y-6" onSubmit={handleSubmit}>
            {error && (
              <div className="text-red-600 text-sm font-medium bg-red-50 p-3 rounded-md border border-red-100 animate-pulse">
                {error}
              </div>
            )}
            ...
            <button 
              type="submit" 
              className="w-full flex justify-center py-2.5 px-4 border border-transparent rounded-lg shadow-sm text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors cursor-pointer"
            >
          ...
          <div className="mt-6 pt-4 border-t border-slate-100">
            <button 
              ...
              className="w-full text-center text-sm text-slate-500 hover:text-indigo-600 font-semibold transition-colors cursor-pointer"
            >
